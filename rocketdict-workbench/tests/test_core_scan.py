...
def test_known_03032_exact_filename_with_wrong_bytes_fails_identity(tmp_path: Path) -> None:
    ...
    archive = _zip(
        source,
        tmp_path / "RocketDict_0.30.32_LAB_STAGE6W_OFFLINE_COMPACTION_COMPLETE.zip",
    )
    report = scan_core_candidates(archive)
    match = report["candidates"][0]["historical_checkpoint_matches"][0]
...
def test_known_0308_name_with_wrong_bytes_fails_exact_identity_match(tmp_path: Path) -> None:
    ...
    match = scan_core_candidates(archive)["candidates"][0]["historical_checkpoint_matches"][0]
...
def test_unproven_03034_filename_is_not_guessed_as_stage6y_checkpoint(tmp_path: Path) -> None:
    source = _candidate(tmp_path / "source", "0.30.34")
    archive = _zip(source, tmp_path / "RocketDict_0.30.34.zip")
    row = scan_core_candidates(archive)["candidates"][0]
