...
This is deliberately not a generic prime-notation fallback.  The immutable
source must contain exactly one ``D deg. M'. S''`` expression, contain the word
``Angle``, and remain a short statement (<=12 alphabetic words).  That source
complexity gate excludes longer optics sentences where a mechanically clean
TC-big candidate can still mistranslate technical content (for example
``Chord``).  A raw candidate additionally has to preserve the exact D/M/S
values and Russian angular anchors ``угол`` + ``град``.
"""
...
TC_BIG_SHORT_ANGULAR_DMS_RESCUE_CONTRACT = "rocketdict-stage12-tc-big-short-angular-dms-rescue/1"
TC_BIG_SHORT_ANGULAR_DMS_SELECTOR_CONTRACT = "rocketdict-stage12-tc-big-short-angular-dms-selector/1"
TC_BIG_SHORT_ANGULAR_DMS_TRIGGER_CONTRACT = "rocketdict-stage12-tc-big-short-angular-dms-trigger/1"
TC_BIG_SHORT_ANGULAR_DMS_SELECTED_PHASE = "tc-big-short-angular-dms-selected-v1"
...
def _prime_verdict(verdict: dict[str, Any]) -> dict[str, Any]:
    return dict(((verdict.get("numeric_symbol") or {}).get("numeric") or {}).get("prime_notation") or {})
...
def evaluate_tc_big_short_angular_dms_trigger(
    ...
    expected_signature = [] if dms is None else [
...
def evaluate_tc_big_short_angular_dms_candidate(source: str, target: str) -> dict[str, Any]:
    dms = _source_dms(source)
    verdict = evaluate_rescue_pair(source, target)
    prime = _prime_verdict(verdict)
...
def _eligible_rows(*, content: str, base_rows: list[dict[str, Any]], context_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in sorted(base_rows, key=lambda item: int(item["source_start"])):
        ...
        trigger = evaluate_tc_big_short_angular_dms_trigger(row, exact_stage10_context=exact_context)
...
def run_stage12(
    ...
    *,
    ...
    if str(effective.get("tc_big_short_angular_dms_rescue_contract") or TC_BIG_SHORT_ANGULAR_DMS_RESCUE_CONTRACT) != TC_BIG_SHORT_ANGULAR_DMS_RESCUE_CONTRACT:
        raise StageExecutionError("Unsupported TC-big short-angular-DMS rescue contract")
    if str(effective.get("tc_big_short_angular_dms_selector_contract") or TC_BIG_SHORT_ANGULAR_DMS_SELECTOR_CONTRACT) != TC_BIG_SHORT_ANGULAR_DMS_SELECTOR_CONTRACT:
        raise StageExecutionError("Unsupported TC-big short-angular-DMS selector")
    if str(effective.get("tc_big_short_angular_dms_trigger_contract") or TC_BIG_SHORT_ANGULAR_DMS_TRIGGER_CONTRACT) != TC_BIG_SHORT_ANGULAR_DMS_TRIGGER_CONTRACT:
        raise StageExecutionError("Unsupported TC-big short-angular-DMS trigger")
    if effective.get("tc_big_short_angular_dms_rescue_phase") not in {None, TC_BIG_SHORT_ANGULAR_DMS_SELECTED_PHASE}:
        raise StageExecutionError("tc_big_short_angular_dms_rescue_phase is internal and may not be overridden")

    effective["enable_tc_big_short_angular_dms_rescue"] = True
    effective["tc_big_short_angular_dms_rescue_contract"] = TC_BIG_SHORT_ANGULAR_DMS_RESCUE_CONTRACT
    effective["tc_big_short_angular_dms_selector_contract"] = TC_BIG_SHORT_ANGULAR_DMS_SELECTOR_CONTRACT
    effective["tc_big_short_angular_dms_trigger_contract"] = TC_BIG_SHORT_ANGULAR_DMS_TRIGGER_CONTRACT
    effective["tc_big_short_angular_dms_rescue_phase"] = TC_BIG_SHORT_ANGULAR_DMS_SELECTED_PHASE
    ...
    try:
        ...
        if attempts:
            ...
            if runtime_status.get("available") is not True:
                raise StageExecutionError(f"TC-big short-angular-DMS runtime unavailable: {runtime_status}")
            ...
            if len(generated) != len(attempts) or any(len(hypotheses) != NUM_HYPOTHESES for hypotheses in generated):
                raise StageExecutionError("TC-big short-angular-DMS n-best cardinality drift")
        ...
        if "".join(str(row["source_text"]) for row in final_rows) != content:
            raise StageExecutionError("TC-big short-angular-DMS final source coverage is not byte-exact")
        ...
        if source_sum != sum(len(str(row.get("source_text") or "")) for row in base_rows):
            raise StageExecutionError("TC-big short-angular-DMS changed total source coverage")
        ...
        output = {
            **dict(base_output),
            ...
            "tc_big_short_angular_dms_rescue_attempted_source_starts": [int(attempt["row"]["source_start"]) for attempt in attempts],
            "tc_big_short_angular_dms_rescue_accepted_source_starts": [int(value["source_start"]) for value in accepted_values],
            "tc_big_short_angular_dms_rescue_selected_ranks": [int(value["selected_rank"]) for value in accepted_values],
            "tc_big_short_angular_dms_rescue_selected_targets": [str(value["target_text"]) for value in accepted_values],
